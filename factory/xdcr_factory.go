...
func (xdcrf *XDCRFactory) constructOutgoingNozzles(spec *metadata.ReplicationSpecification, kv_vb_map map[string][]uint16,
	...
	for kvaddr, kvVBList := range kvVBMap {
		...
		//check for msg nozzletype
		isMsgNozzle := (nozzleType == base.Msg)
		...
		for i := 0; i < numOfOutNozzles; i++ {
			...
			if isCapiNozzle {
			    ...
			    outNozzle, err = xdcrf.constructMSGNozzle(spec.Id, targetClusterRef.UserName, targetClusterRef.Password, targetClusterRef.Certificate, vbList, i, logger_ctx)
				if err != nil {
					return nil, nil, err
				}
			} else {
...
func (xdcrf *XDCRFactory) getOutNozzleType(targetClusterRef *metadata.RemoteClusterReference, spec *metadata.ReplicationSpecification) (base.XDCROutgoingNozzleType, error) {
	...
	case metadata.ReplicationTypeMsg:
		return base.Msg, nil
...
func (xdcrf *XDCRFactory) constructMSGNozzle(topic string,
		username string,
		password string,
		certificate []byte,
		vbList []uint16,
	...
	if len(vbList) == 0 {
		// should never get here
		xdcrf.logger.Errorf("Skip constructing MSG nozzle with index %v since it contains no vbucket", nozzle_index)
	}

	xdcrf.logger.Debugf("Construct MessagingNozzle: topic=%s, kvaddr=%s", topic, "localhost")
	// partIds of the messaging nozzles look like "msg_$topic_$kvaddr_1"
	msgNozzle_Id := xdcrf.partId(MSG_NOZZLE_NAME_PREFIX, topic, "localhost", nozzle_index)
	nozzle := parts.NewMessagingNozzle(msgNozzle_Id, topic, "localhost", username, password, certificate, logger_ctx)
	return nozzle, nil	
